...
import ontey.api.config.util.DumperBus;
...
public class YamlCommentDumper extends YamlCommentReader {
	 ...
	 * <br>The result is written to the writer.</p>
	 *
	...
	public void getCommentNode(KeyTree.Node readerNode) {
		if(readerNode != null) {
			...
			if(this.commentNode != null) {
				if(this.commentNode.getParent() != null && this.commentNode.getParent().isList() && this.commentNode.size() == 1) {
					this.checkFirstListMapElement(this.commentNode, readerNode); // first key for list maps
				}
			}
			
			if(this.commentNode == null ||
			  (readerNode.getName() != null && (this.commentNode.getComment() == null || this.commentNode.getSideComment() == null))) {
				if(readerNode.getParent() != null && readerNode.getParent().isList() && readerNode.getElementIndex() != null) {
