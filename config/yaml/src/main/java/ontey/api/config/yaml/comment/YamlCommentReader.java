...
public abstract class YamlCommentReader extends YamlCommentMapper implements Closeable {
	...
	protected YamlCommentReader(@NonNull YamlConfigOptions options) {
		super(options);
	}
	...
	protected void processLine() throws IOException {
		// overridden in children to process the current line
	}
	
	@SuppressWarnings("unused")
	protected void processMultiline(boolean inQuoteBlock) throws IOException {
		// overridden in children to process the current line while reading a multiline value
	}
	
	@SuppressWarnings("unused")
	protected void endExplicitBlock() throws IOException {
		// overridden in children to process an explicit block
	}
	...
	protected void checkSpecialLines(boolean wasLiteral, int indent) throws IOException {
		...
		if(this.currentList != null) {
			int currentListIndent = this.currentList.getIndent();
	...
	protected boolean isSectionEnd() {
		return this.currentNode != null && this.indent <= (this.currentNode.getIndent() - this.options().indent());
	}
	...
	protected void trackListElement() {
		if(this.currentList == null || (this.currentNode != null && this.indent > this.currentNode.getIndent())) {
			this.currentList = this.keyTree.findParent(this.indent + 2); // "- " prefix
			if(this.currentList.getListSize() == null || this.currentList.size() == 0) {
		...
		this.currentList.isList(this.currentList.getListSize() + 1);
		this.currentNode.setElementIndex(this.currentList.getListSize() - 1);
	}
