...
import ontey.api.config.ConfigOptions;
...
public class KeyTree implements Iterable<KeyTree.Node> {
	...
	public KeyTree(@NonNull ConfigOptions options) {
	 ...
	 *
	 * @param path the path of names to look for separated by {@link #options()} {@link ConfigOptions#pathSeparator()}
	...
	public class Node implements Iterable<KeyTree.Node> {
		...
		protected Node(KeyTree.Node parent, int indent, String name) {
			this.parent = parent;
			this.indent = indent;
			this.name = name;
		}
		 ...
		 *
		 * @param path the path of children names to look for separated by {@link #options()} {@link ConfigOptions#pathSeparator()}
			...
			if(value != null) {
				if(value instanceof Collection) {
					...
					if(value instanceof ConfigSection) {
						value = ((ConfigSection) value).getValues(false);
					}
