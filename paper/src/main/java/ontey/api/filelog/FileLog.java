...
public class FileLog {
	...
	public FileLog(@NonNull NamedLogger logger, @NonNull File dataFolder) {
		this.LOGGER = logger;
		
		LOGGER.debug("Loading FileLog");
		
		directory = new File(dataFolder, "logs");
	...
	public FileLog(@NonNull String name) {
		this(new NamedLogger(name), Path.of("/plugins", name).toFile());
	}
	
	public FileLog(@NonNull String name, @NonNull File dataFolder) {
		this(new NamedLogger(name), dataFolder);
	}
